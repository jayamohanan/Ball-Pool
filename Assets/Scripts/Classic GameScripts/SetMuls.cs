...
using UnityEngine;
public class SetMuls : MonoBehaviour
{
    ...
    public void CreateMuls()
    {
        ...
        if (gameManager.currentLevel > 20 && gameManager.currentLevel % 5 == 0)
        {
            ...
            else if (rnd < 3)
            {
                for (int i = 0; i < mulCount - 1; i++)
                {
                    SpawnMulType(MulType.poleVault, position, i);
                }
            }
            else if (rnd < 4)
            {
        ...
        else
        {
            for (int i = 0; i < mulCount - 1; i++)
            {
                ...
                //    //position += Vector3.forward * forwardGap;
                //    //SpawnSingleMul(MulType.poleVault, position, i);
                ...
                //    SpawnTwoMulWithObstacles(MulType.normal, position, i, Vector3.zero);
                //    continue;
                if ((gameManager.currentLevel < 3) || i == 0 || rand < 1)
                {
                ...
                else if (rand < 3)//forward
                {
                    SpawnMulType(MulType.forward, position, i);
                }
                else if (rand < 4)//backward
                {
                    SpawnMulType(MulType.backward, position, i);
                }
                else if (rand < 5)
                ...
                //else if (i == 6)
                //{
                //    position = SetDistance(MulType.poleVault, position);
                //    SpawnSingleMul(MulType.poleVault, position, i);
                //}
                //else if (i == 7)
                //{
    ...
    private Vector3 SetDistance(MulType mulType, Vector3 position)
    {
        ...
        if (mulType == MulType.normal || mulType == MulType.sphere || mulType == MulType.sphereButterfly || mulType == MulType.bigOne || mulType == MulType.tough || mulType == MulType.poleVault || mulType == MulType.upway || mulType == MulType.horizontal || mulType == MulType.butterfly)
        {
            if (prevMulType == MulType.forward)
            {
                position += Vector3.forward * 25;
    ...
    public enum MulType
    {
        normal, tough, forward, backward, horizontal, upway, downway, poleVault, butterfly, bigOne, sphere, sphereButterfly, none
    }
    ...
    {
        return (mulType == MulType.poleVault || mulType == MulType.butterfly || mulType == MulType.bigOne || mulType == MulType.sphere || mulType == MulType.sphereButterfly);
    }
